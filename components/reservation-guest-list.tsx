...
export function ReservationGuestList({
    ...
    const renderField = (value: string | undefined, placeholder: string) => {
        ...
        return <span className="text-[#E5E5E5]">{value}</span>
    }
    ...
    return (
        <Card className="bg-[#1A1A1A] border-[#333333] shadow-sm h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-[#333333]">
                <div className="space-y-1">
                    <CardTitle className="text-base font-bold text-[#E5E5E5] flex items-center gap-2">
                        <User className="h-4 w-4 text-[#D4AF37]" /> Lista de Ocupantes
                    </CardTitle>
                    <CardDescription className="text-xs text-[#A3A3A3]">
                        Ocupación: <span className={cn("font-medium", isLimitReached ? "text-red-400" : "text-[#E5E5E5]")}>
                            {guests.length}
                        </span> de <span className="text-[#E5E5E5] font-medium">{maxGuests}</span> permitidos
                    </CardDescription>
                </div>
                <Button
                    size="sm"
                    ...
                    className={cn(
                        ...
                        isLimitReached
                            ? "border-[#333333] text-[#555] cursor-not-allowed bg-transparent"
                            : "border-[#D4AF37] text-[#D4AF37] bg-[#D4AF37]/5 hover:bg-[#D4AF37]/10"
                    )}
                    ...
                    {guests.map((guest, index) => {
                        ...
                        return (
                            <AccordionItem
                                key={index}
                                value={`guest-${index}`}
                                className="border border-[#333333] rounded-lg bg-[#262626] overflow-hidden transition-all hover:border-[#555]"
                            >
                                <AccordionTrigger className="hover:no-underline px-4 py-3 data-[state=open]:bg-[#1A1A1A]">
                                    <div className="flex items-center gap-4 w-full text-left pr-4">
                                        <Avatar className="h-10 w-10 border border-[#333333]">
                                            <AvatarFallback className="bg-[#0F0F0F] text-[#D4AF37] font-bold text-xs">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <p className="font-bold text-sm text-[#E5E5E5] truncate">
                                                    {guest.primerNombre} {guest.primerApellido}
                                                </p>
                                                {guest.esTitular && (
                                                    <Badge variant="secondary" className="bg-[#D4AF37]/20 text-[#D4AF37] border-0 text-[9px] px-1.5 h-4">
                                                        TITULAR
                                                    </Badge>
                                                )}
                                            </div>
                                ...
                                <AccordionContent className="bg-[#1A1A1A] border-t border-[#333333] px-4 py-4">
                                    <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div className="bg-[#262626] p-2 rounded border border-[#333333] flex items-center gap-3">
                                                <div className="h-8 w-8 rounded-full bg-[#1A1A1A] flex items-center justify-center text-[#A3A3A3]">
                                                    <Mail className="h-4 w-4" />
                                                </div>
                                                <div className="overflow-hidden w-full">
                                                    <p className="text-[10px] text-[#737373] uppercase">Correo</p>
                                                    <p className="text-xs text-[#E5E5E5] truncate" title={guest.correo}>
                                            ...
                                            <div className="bg-[#262626] p-2 rounded border border-[#333333] flex items-center gap-3 md:col-span-2">
                                                <div className="h-8 w-8 rounded-full bg-[#1A1A1A] flex items-center justify-center text-[#A3A3A3]">
                                        ...
                                        <div className="flex items-center gap-2 pt-2">
                                            <Button
                                                ...
                                                size="sm"
                                                className={cn(
                                                    "flex-1 border-[#333333] text-xs h-8",
                                                    isSigned
                                            ...
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                                className="h-8 w-8 p-0 text-[#A3A3A3] hover:text-[#E5E5E5]"
                                            ...
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-[#A3A3A3] hover:text-[#E5E5E5]">
                                                        ...
                                                    </Button>
                                                </DropdownMenuTrigger>
