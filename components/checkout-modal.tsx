...
import {
  ...
  Users,
  ArrowLeft,
  AlertTriangle // Importamos icono de alerta
} from "lucide-react"
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { cn } from "@/lib/utils"
import { useCashierStore } from "@/lib/store" // Importamos el store de caja
...
export function CheckoutModal({ isOpen, onClose, total, activeFolios, onComplete }: CheckoutModalProps) {
  const { isShiftOpen } = useCashierStore() // Obtenemos estado de la caja
      ...
      <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
        <DialogContent className="sm:max-w-[500px] bg-card border-border p-0 [&>button]:hidden">
          ...
          <DialogHeader className="p-6 pb-4 border-b border-border">
            ...
            {/* Solo mostramos el total si la caja está abierta */}
            {isShiftOpen && (
          ...
          <div className="p-6">
            ...
            {!isShiftOpen ? (
                <div className="flex flex-col items-center justify-center py-6 text-center space-y-4 animate-in zoom-in-95">
                  <div className="h-20 w-20 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center ring-1 ring-red-200 dark:ring-red-800">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground max-w-[280px] mx-auto mt-2">
                      No es posible procesar pagos o cargos sin un turno de caja abierto.
                    </p>
                  </div>
                ...
                // --- FLUJO NORMAL DE PAGO (Si la caja está abierta) ---
                <>
                  {paymentMethod === null ? (
